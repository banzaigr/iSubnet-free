...
**Target build: versionCode/CURRENT_PROJECT_VERSION 73, versionName/MARKETING_VERSION 1.1.16.** This supersedes the version numbers in the round as originally drafted — see "Corrections made" below. Verified directly against the uploaded project as of build 70 (uploaded 2026-09-16); do not re-derive these numbers from an older copy of this prompt.

**Two builds ship ahead of this round, separately.** Build 71 (iOS already uploaded to TestFlight) shipped a first pass of the mobile keyboard `inputmode` fix, but that pass had a usability regression — it dropped `inputmode` entirely on fields that should keep a numeric keypad, downgrading them to a full alphanumeric keyboard. Build 72 is the corrected version of that fix (restores `inputmode="decimal"` on the two fields that need it, plus a small JS normalizer and an updated parsing regex — see the standalone build-72 prompt). Build 72 ALSO bumped versionName/MARKETING_VERSION from 1.1.15 to **1.1.16** on both platforms, because Apple has since approved/released 1.1.15, and a marketing version that's already live can't be reused for a further submission. This round therefore starts from a build-72 baseline (Android versionCode 72, iOS CURRENT_PROJECT_VERSION 72, versionName/MARKETING_VERSION 1.1.16 on both platforms) and produces build 73, staying on 1.1.16 (do not bump versionName again unless 1.1.16 itself gets approved/released before this round ships — if that happens, treat it exactly like the 1.1.15 situation and move to 1.1.17 instead). Neither keyboard fix is part of this round — do not redo either here, and do not reference them as "Part G" of this document; they already shipped separately.
...
1. **Version numbers corrected (Task E1), then corrected twice more for the two keyboard-fix builds.** The round as originally drafted said to bump versionCode 67→68. The project moved on (build 70 uploaded, at versionCode 70 on Android), so this was first corrected to 70→71. Build 71 then shipped (iOS to TestFlight) as a standalone keyboard fix, then needed its own correction as build 72 (see the note at the top of this document). So this round now bumps **Android versionCode 72→73** (versionName stays "1.1.16", already current, no change needed).

2. **iOS was further behind than Android — builds 71/72 already closed that gap.** `ios/App/App.xcodeproj/project.pbxproj` was originally found at `CURRENT_PROJECT_VERSION = 67` and `MARKETING_VERSION = 1.1.14` (both occurrences), never bumped to match Android's build 68/69/70 rounds. The build-71 and build-72 keyboard-fix releases are responsible for bringing iOS up to `CURRENT_PROJECT_VERSION = 72` / `MARKETING_VERSION = 1.1.16`. By the time this round starts, iOS and Android should already be numbered identically at 72 — this round's Task E1 is therefore a normal, single-step 72→73 bump on both platforms, not a catch-up jump.

3. **versionName/MARKETING_VERSION is now 1.1.16, not 1.1.15 — a second, separate correction from Task E1's original scope.** Apple approved/released marketing version 1.1.15 during the build-72 round, and a marketing version that's already live on the App Store cannot be reused for a further submission — so build 72 bumped versionName (Android) and MARKETING_VERSION (iOS) from 1.1.15 to 1.1.16 on both platforms, for consistency across stores. This round keeps 1.1.16 unchanged (see Task E1) unless 1.1.16 itself gets approved/released before this round ships, in which case apply the same logic again and move to 1.1.17 instead.

4. **Part D has a hard blocker.** Checked for both Firebase config files — neither exists in the uploaded project: `android/app/google-services.json` is missing and `ios/App/App/GoogleService-Info.plist` is missing. Per Task D0's own instruction, this means **Part D (Crashlytics) cannot start** until these are downloaded from the Firebase console and added to the project. All other parts (A, B, C, E, F) are unaffected and should proceed regardless.
...
- **Before starting this task, confirm the build-72 baseline is actually in place**: `android/app/build.gradle` should read `versionCode 72` / `versionName "1.1.16"`, and `ios/App/App.xcodeproj/project.pbxproj` should read `CURRENT_PROJECT_VERSION = 72` / `MARKETING_VERSION = 1.1.16` (both occurrences on iOS). If build 72 hasn't shipped yet or these don't match — especially if versionName/MARKETING_VERSION still says 1.1.15 instead of 1.1.16 — stop and confirm with the team before proceeding. (1.1.15 was bumped to 1.1.16 during build 72 because Apple had already approved/released 1.1.15; if that bump didn't actually land, apply it now before continuing.)
- **Android:** `android/app/build.gradle` — bump `versionCode` from **72 to 73**. `versionName` stays `"1.1.16"` — already current, no change.
- **iOS:** `ios/App/App.xcodeproj/project.pbxproj` — bump `CURRENT_PROJECT_VERSION` from **72 to 73** (both occurrences). `MARKETING_VERSION` stays `1.1.16` — already correct, no change needed.
- Do NOT touch `package.json`'s `"version"` field — cosmetic, unused by either store. Confirm it reads `"1.1.16"` to match; update it if build 72 left it stale at `"1.1.15"` (it has no functional effect on either store, but keep it in sync for anyone reading the file).
- Update the version footer text in `index.html` (should read `iSubnet v1.1.16 (Build 72)` once the build-72 keyboard fix has landed) to **`iSubnet v1.1.16 (Build 73)`**.
...
**Part E:** confirmed the build-72 baseline (versionCode/CURRENT_PROJECT_VERSION 72, versionName/MARKETING_VERSION 1.1.16 on both platforms) was in place before bumping; Android `versionCode` = 73, `versionName` = "1.1.16"; iOS `CURRENT_PROJECT_VERSION` = 73, `MARKETING_VERSION` = 1.1.16 (both occurrences on both platforms); version footer reads "iSubnet v1.1.16 (Build 73)"; `package.json` version reads "1.1.16" for consistency.
