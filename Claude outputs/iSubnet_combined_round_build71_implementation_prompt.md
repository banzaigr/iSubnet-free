...
1. **Version numbers corrected (Task E1).** The round as originally drafted said to bump versionCode 67→68 and versionName "1.1.14"→"1.1.15". The project has since moved on (build 70 is already uploaded, at versionCode 70 / versionName "1.1.15" on Android). Bumping to 68 would be silently accepted by nothing — Google Play rejects any versionCode that doesn't strictly increase — so this round now bumps **Android versionCode 70→71** (versionName stays "1.1.15", already current, no change needed).

2. **iOS version numbers are further behind than Android and need a bigger jump.** Checked `ios/App/App.xcodeproj/project.pbxproj` directly: it is still at `CURRENT_PROJECT_VERSION = 67` and `MARKETING_VERSION = 1.1.14` (both occurrences, Debug and Release configs) — it was never bumped to match Android's build 68/69/70 rounds. Task E1 below has this jump to CURRENT_PROJECT_VERSION 71 / MARKETING_VERSION 1.1.15 in one step, not 67→68.
...
- **Android:** `android/app/build.gradle` — bump `versionCode` from **70 to 71**. `versionName` stays `"1.1.15"` — already current, no change.
- **iOS:** `ios/App/App.xcodeproj/project.pbxproj` — bump `CURRENT_PROJECT_VERSION` from **67 to 71** (both occurrences — verified at lines 299 and 321) and `MARKETING_VERSION` from **1.1.14 to 1.1.15** (both occurrences — verified at lines 306 and 328). iOS was left behind on earlier rounds and needs this larger jump to catch up to Android.
...
- Update the version footer text in `index.html` (verified: line 272, currently reads `iSubnet v1.1.15 (Build 70)`) to **`iSubnet v1.1.15 (Build 71)`**.
...
**Part E:** Android `versionCode` = 71, `versionName` = "1.1.15"; iOS `CURRENT_PROJECT_VERSION` = 71, `MARKETING_VERSION` = 1.1.15 (both occurrences on both platforms); version footer reads "iSubnet v1.1.15 (Build 71)"; `package.json` version left untouched at "1.1.15".
